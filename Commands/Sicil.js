const { MessageEmbed, splitEmbedWithDesc } = require("discord.js");
...
module.exports.execute = async(client, message, args,  emoji) => {
  ...
  let durum;
    ...
    if (cpuan == null || cpuan == undefined) durum = "Çok Güvenli";
  sicil = sicil.reverse();
  let listedPenal = sicil.length > 0 ? sicil.map((value, index) => `\`${index + 1}.\` **[${value.Tip}]** ${new Date(value.Zaman).toTurkishFormatDate()} tarihinde **${value.Sebep}** nedeniyle ${message.guild.members.cache.has(value.Yetkili) ? message.guild.members.cache.get(value.Yetkili) : value.Yetkili} tarafından cezalandırıldı. (\`#${value.id}\`)`).join("\n") : "Temiz!";
  client.splitEmbedWithDesc(`**${uye} Üyesinin Sicili** (\`${durum}\`) \n\n ${listedPenal}`,
                           {name: message.guild.name, icon: message.guild.iconURL({dynamic: true, size: 2048})},
                           {name: "ABEY ❤️", icon: false},
