...
module.exports.execute = async (client, message, args, emoji) => {
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor(client.randomColor()).setTimestamp().setFooter("δ GLADIO").setTitle('Üye Banlandı!').setDescription(`**Banlayan Yetkili:** ${message.author} (${message.author.id})\n**Banlanan Üye:** ${kisi.tag} (${kisi.id})\n**Sebep:** ${reason}`));
    } else {
  ...
  if (banLimit >= 5) {
    ...
    message.member.roles.remove(ayar.banciRolleri).catch();
  ...
  message.channel.send(embed.setImage("https://i.pinimg.com/originals/b2/84/33/b28433c392959f923ff0d736cd89dcbd.gif").setDescription(`\`${victim.user.tag}\` üyesi ${message.author} tarafından **${reason}** nedeniyle **banlandı!** (\`#${cezaID}\`)`));
  webhook.send(new MessageEmbed().setTimestamp().setFooter("δ GLADIO").setTitle('Üye Banlandı!').setDescription(`

 Ceza Uygulayan : ${message.author}(\`${message.author.id}\`)
 Ceza Alan : ${victim.user.tag}(\`${victim.user.id}\`)
 Sebep : **${reason}**
 Ceza İşlemi : Ban (\`#${cezaID}\`)`)).catch();
};
