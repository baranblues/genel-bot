...
module.exports.onLoad = async (client) => {
    ...
    client.on("message", async message => {
      ...
      if (nextLevel <= currentlyData.XP) {
        currentlyData.Level++;
        currentlyData.XP = 0;
       const id = conf.levellog;
       const channel = user.guild.channels.cache.get(id);
        channel.send(
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    let embed = new MessageEmbed().setThumbnail(user.user.avatarURL({dynamic: true, size: 2048})).setAuthor(user.displayName, user.user.avatarURL({dynamic: true, size: 2048})).setFooter("δ GLADIO").setColor(client.randomColor()).setTimestamp();
