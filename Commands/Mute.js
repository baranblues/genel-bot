...
module.exports.execute = async (client, message, args, emoji) => {
  ...
  if (!muteler.some(j => j.id == uye.id)) {
    ...
    jdb.push(`tempmute`, {id: uye.id, kalkmaZamani: Date.now()+ms(sure), id: cezaID})
 ...
 let cezaID = jdb.get(`cezaid.${message.guild.id}`)+1
 message.channel.send(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${sure}** boyunca **${reason}** nedeniyle mutelendi!(\`#${cezaID}\`)`)).catch();
 webhook.send(embed.setDescription(`
 Ceza Uygulayan : ${message.author} (\`${message.author.id}\`)
 Ceza Alan : ${uye} (\`${uye.id}\`)
 Sebep : **${reason}**
 Süre : **${sure}**
 Ceza İşlemi : Chat Mute (\`#${cezaID}\`)`)).catch();
};
module.exports.configuration = {
  name: "mute",
  aliases: ['mute', 'sustur', 'geçici-mute'],
